...
if __name__ == "__main__":
    ...
    msg_usd = currency_exchange_rate(currency_from="USD", currency_to="RUB")
    time.sleep(5)
    # msg2 = '' # fx_intraday(currency_from="USD", currency_to="RUB", interval = "15min", outputsize="compact")
    msg_btc = currency_exchange_rate(currency_from="BTC", currency_to="USD")
    time.sleep(5)
    msg_eur = currency_exchange_rate(currency_from="EUR", currency_to="RUB")

    tlg.send_msg(msg_usd + "\n#usd")
    tlg.send_msg(msg_eur + "\n#eur")
    tlg.send_msg(msg_btc + "\n#btc")
